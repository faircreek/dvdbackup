/*
 * Copyright (C) 2002-2015 Olof <olof.faircreek@gmail.com>
 *
...
/* Usage:
	...
	General backup information:
		...
		means that every file will have the same size as
		the original one. Likewise goes also for the -F
		and the -T switch.
		...
		original one since there may be gaps in the cell
	...
	To backup the main feature of the DVD:
		...
		dvdbackup does it's best to make a intelligent guess
	...
	To backup a specific chapter or chapters from a title:
		...
		(cell) and on other.dvdbackup will backup all sectors that belong
	...
	Todo - i.e. what's on the agenda.
		...
		it's job, but it's implementation isnt that great. I would also like
...
void usage(){
	...
	fprintf(stderr,"\t-M\t\tbackup the whole DVD (mirror)\n");
...
int DVDWriteCells(dvd_reader_t * dvd, int cell_start_sector[], int cell_end_sector[],
   int length, int titles, title_set_info_t * title_set_info, titles_info_t * titles_info, char * targetdir,char * title_name){
	...
	/* Remove all old files silently if they exist */
	...
	for (f = 0; f < length ; f++) {
		...
		if ((buffer = (unsigned char *)malloc(buff * 2048 * sizeof(unsigned char))) == NULL) {
		        fprintf(stderr, "Out of memory (RAM) copying %s\n", targetname);
			return(1);
		}
		...
		while( left > 0 ) {
			...
			if ( DVDReadBlocks(dvd_file,soffset,buff, buffer) != buff) {
				fprintf(stderr, "Error reading TITLE VOB\n");
				free(buffer);
...
titles_info_t * DVDGetInfo(dvd_reader_t * _dvd) {
	...
	/* Iterate over vmg_nr_of_title_sets */
	...
	/* Here we will take the biggest/second and if that is bigger than one it's not a feature title */
	...
	if ( CheckSizeArray(size_size_array, 0, 1)  == 1 ) {
		...
		if (  vmg_ifo->vts_atrt->vts[title_set_size_array[0] - 1].vtstt_vobs_video_attr.display_aspect_ratio ==
			...
			They are within the same size range and have the same aspect ratio
			...
			if ( vmg_ifo->vts_atrt->vts[title_set_size_array[0] - 1].vtstt_vobs_video_attr.display_aspect_ratio == aspect) {
				...
				/* Okay we didn't have the preferred aspect ratio - just make the biggest one a candidate */
	...
	/* Lets start checking audio,sub pictures and channels. My guess is that a special suburb will make dvd's with titles with a lot of
	...
	/* Here we take chapters into consideration*/
	if (found == 3) {
		FreeSortArrays( chapter_chapter_array, title_set_chapter_array,
				angle_angle_array, title_set_angle_array,
				subpicture_sub_array, title_set_sub_array,
	...
	/* Some random thoughts about DVD guessing */
	...
	/* Make a tripple array with chapters, angles and title sets
	 - sort out dual title sets with a low number of chapters. Traditionally
	 the title set with most chapters is the main film. Number of angles is
	 kept as a reference point of low value*/
	...
	/* Make a dual array with number of audio streams, sub picture streams
	 ...
	 since it's supposed be syncronised e.g. a English film syncronised/dubbed
	 in German. We are also keeping track of subtitles since it's also an 
	 indication of the main film*/
	...
	/* Which title set is belonging to title 1 and how many chapters has it. Once
	 again traditionally title one is belonging to the main film*/
	...
	/* Okay lets see if the biggest one has most chapters, it also has more subtitles
	 ...
	 Now we fetch the 16:9 by default unless we're forced to do 4:3
	 First check which one is which.
	 If the 16:9 is the biggest one and has the same or more subtiles, audio streams
	 ...
	 two biggest once are really big and possibly carry the same film in different formats.

	 We will now return the preferred one either 16:9 or 4:3 but we will first check that the one
	 we return at least has two or more audio tracks. We don't want it if the other one has a lot
	 more sound (we may end up with a film that only has 2ch Dolby Digital so we want to check for
	 6ch DTS or Dolby Digital. If the preferred one doesn't have those features but the other once has
...
int DVDCopyTileVobX(dvd_reader_t * dvd, title_set_info_t * title_set_info, int title_set, int vob, char * targetdir,char * title_name) {
...
#ifdef DEBUG
	fprintf(stderr,"After we check the vob itself %d\n", vob);
#endif
	...
	/* Now figure out the offset we will start at. Also check that the previous files are of valid DVD size */
	for ( i = 0; i < vob - 1; i++ ) {
		tsize = title_set_info->title_set[title_set].size_vob[i];
		if (tsize%2048 != 0) {
			fprintf(stderr, "The Title VOB number %d of title set %d doesn't have a valid DVD size\n", i + 1, title_set);
	...
	if (stat(targetname, &fileinfo) == 0) {
		fprintf(stderr, "The Title file %s exists. Will try to overwrite it.\n", targetname);
...
int DVDCopyMenu(dvd_reader_t * dvd, title_set_info_t * title_set_info, int title_set, char * targetdir,char * title_name) {
	...
	if (stat(targetname, &fileinfo) == 0) {
		fprintf(stderr, "The Menu file %s exists. Will try to overwrite it.\n", targetname);
		if (! S_ISREG(fileinfo.st_mode)) {
...
int DVDCopyIfoBup (dvd_reader_t * dvd, title_set_info_t * title_set_info, int title_set, char * targetdir,char * title_name) {
	...
	if (stat(targetname, &fileinfo) == 0) {
		fprintf(stderr, "The IFO file %s exists. Will try to overwrite it.\n", targetname);
		if (! S_ISREG(fileinfo.st_mode)) {
	...
	if ((buffer = (unsigned char *)malloc(size * sizeof(buffy)/*90-60-90?*/)) == NULL) {
		fprintf(stderr, "Out of memory copying %s\n", targetname);
		close(streamout);
		return(1);
	}
	...
	if (stat(targetname, &fileinfo) == 0) {
		fprintf(stderr, "The BUP file %s exists. Will try to overwrite it.\n", targetname);
		if (! S_ISREG(fileinfo.st_mode)) {
	...
	if ( title_sets >= 1 ) {
 		for (counter=0; counter < title_sets; counter++ ){
			...
			if (title_set_info->title_set[counter +1].size_ifo != title_set_info->title_set[counter + 1].size_bup) {
				fprintf(stderr,"BUP and IFO size for fileset %d is not the same! Be warned!\n", counter + 1);
			}
...
int DVDMirrorTitleSet(dvd_reader_t * _dvd, char * targetdir,char * title_name, int title_set) {
	...
	if ( title_set > title_set_info->number_of_title_sets ) {
		fprintf(stderr, "Can't copy title_set %d, there are only %d title_sets present on this DVD\n", title_set, title_set_info->number_of_title_sets);
		DVDFreeTitleSetInfo(title_set_info);
		return(1);
	}
...
int DVDMirrorMainFeature(dvd_reader_t * _dvd, char * targetdir,char * title_name) {
	...
	if (!titles_info) {
		fprintf(stderr, "Guesswork to find the main feature film failed\n");
		return(1);
	}
...
int DVDMirrorChapters(dvd_reader_t * _dvd, char * targetdir,char * title_name, int start_chapter,int  end_chapter, int titles) {
	...
	if (end_chapter > titles_info->titles[titles - 1].chapters) {
		...
		fprintf(stderr, "Truncated the end_chapter. Only %d chapters in title %d\n", end_chapter,titles);
	}
	...
	/* Lookup start cell for this pgc/pg */
...
#ifdef DEBUG
		fprintf(stderr,"DVDMirrorChapter: end cell adjusted to %d\n",end_cell);
#endif

	}

#ifdef DEBUG
	fprintf(stderr,"DVDMirrorChapter: start cell %d\n", start_cell);
#endif
